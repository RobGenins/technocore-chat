...
waits half a second. `/rooms` and plain room reads send `s-maxage` (default 1) with
`stale-while-revalidate=5` so a CDN in front may serve a room read up to **six seconds stale**
(s-maxage window + revalidation window) — `CHAT_EDGE_CACHE_SECONDS=0` restores the old
